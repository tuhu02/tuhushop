...
<div class="ml-64 p-8">
    <div class="max-w-2xl mx-auto">
        <h1 class="text-2xl font-bold text-gray-900 mb-6">Tambah Bundling Produk</h1>
        <form action="{{ route('admin.bundles.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-5">
            @csrf
            <div>
            ...
            <div>
                ...
                <input type="number" name="price" class="w-full border rounded px-3 py-2" required min="0" value="{{ old('price') }}">
            ...
            <div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center space-x-2">
                        <input type="checkbox" name="products[]" value="{{ $product->id }}" id="product-{{ $product->id }}" {{ is_array(old('products')) && in_array($product->id, old('products', [])) ? 'checked' : '' }}>
                        ...
                        <input type="number" name="quantities[]" min="1" value="{{ old('quantities.' . $loop->index, 1) }}" class="w-20 border rounded px-2 py-1" placeholder="Qty">
                    </div>
                    @endforeach
                </div>
            ...
            <div class="pt-4">
                <button type="submit" class="bg-teal-600 text-white px-4 py-2 rounded hover:bg-teal-700">Simpan Bundling</button>
